...
<?php while ( $gallery_query->have_posts() ) : $gallery_query->the_post(); ?>
    ...
    <div id="dv-gallery<?php echo esc_attr( $looprandom ); ?><?php the_ID(); ?>" class="dv-gallerycontainer">
		<?php if ( has_post_thumbnail() ) { ?>
			...
			?>
            <div class="dv-galleryimage <?php if ( $vertical == 'yes' ) {
				echo esc_attr( 'vertical' );
			} ?>" data-image="<?php echo esc_url( $thumb_url ); ?>">
                <a class="<?php if ( $gallerytype != 'link' ) {
					echo esc_attr( 'openlightbox' );
				} ?> <?php if ( $gallerytype == 'video' ) {
					echo esc_attr( 'videogal' );
				} ?> <?php if ( $gallerytype == 'link' ) {
					echo esc_attr( 'linkgal' );
				} ?> <?php if ( $vertical == 'yes' ) {
					echo esc_attr( 'vertical' );
				} ?>" href="<?php if ( $gallerytype == 'link' ) {
					echo esc_url( $externallink );
				} else {
					echo esc_attr( '#' );
				} ?>" <?php if ( $newindow == 'on' ) {
					echo 'target="_blank"';
		...
		<?php } ?>
        <div class="dv-gallerycontent <?php if ( ! has_post_thumbnail() ) {
			echo esc_attr( 'withoutfimage' );
		} ?>  <?php if ( $vertical == 'yes' ) {
			echo esc_attr( 'vertical' );
		} ?>">
            <div class="dv-gallerycontent-inner">
                <div class="dvh4"><a href="<?php if ( $gallerytype == 'link' ) {
						echo esc_url( $externallink );
					} else {
						echo esc_attr( '#' );
					} ?>" class="<?php if ( $gallerytype != 'link' ) {
						echo esc_attr( 'openlightbox' );
					} ?>" <?php if ( $newindow == 'on' ) {
						echo 'target="_blank"';
					} ?>><?php the_title(); ?></a></div>
				...
				<?php } ?>
                <a href="<?php if ( $gallerytype == 'link' ) {
					echo esc_url( $externallink );
				} else {
					echo esc_attr( '#' );
				} ?>" class="<?php if ( $gallerytype != 'link' ) {
					echo esc_attr( 'openlightbox' );
				} ?> dv-readmore-button" <?php if ( $newindow == 'on' ) {
					echo 'target="_blank"';
				} ?>><?php esc_attr_e( 'View Gallery', 'dvgallery' ); ?></a>
                <a class="dv-readmore-button ilclg-download-album"
                   href="<?php echo $gallery_download_link; ?>"><?php esc_attr_e( 'Download Gallery', 'dvgallery' ); ?></a>
            </div>
        </div>
    </div>
            ...
            jQuery(document).ready(function ($) {
                jQuery('#dv-gallery<?php echo esc_js( $looprandom ); ?><?php the_ID(); ?> .openlightbox').click(function (e) {
                    ...
                    jQuery(this).lightGallery({
                        dynamic: true,
                        preload: 2,
                        zoom: <?php if ( ! empty( $lgzoom ) ) {
							echo esc_js( $lgzoom );
						} else {
							echo esc_js( 'true' );
						} ?>,
                        fullScreen: <?php if ( ! empty( $lgfullscreen ) ) {
							echo esc_js( $lgfullscreen );
						} else {
							echo esc_js( 'true' );
						} ?>,
                        thumbnail: <?php if ( ! empty( $lgthumbnails ) ) {
							echo esc_js( $lgthumbnails );
						} else {
							echo esc_js( 'true' );
						} ?>,
                        download: <?php if ( ! empty( $lgdownload ) ) {
							echo esc_js( $lgdownload );
						} else {
							echo esc_js( 'true' );
						} ?>,
                        counter: <?php if ( ! empty( $lgcounter ) ) {
							echo esc_js( $lgcounter );
						} else {
							echo esc_js( 'true' );
						} ?>,
                        hideBarsDelay: <?php if ( ! empty( $lghide ) ) {
							echo esc_js( $lghide );
						} else {
							echo esc_js( '6' );
						} ?>000,
                        ...
                        autoplay: true,
                        pause: <?php if ( ! empty( $galleryautoplayduration ) ) {
							echo esc_js( $galleryautoplayduration );
						} else {
							echo esc_js( '4' );
						} ?>000,
						<?php } ?>
	...
	<?php }
	if ( ( $gallerytype == 'video' ) && ( ! empty( $galleryvideos ) ) ) { ?>
        <script type="text/javascript">
            jQuery(document).ready(function ($) {
                jQuery('#dv-gallery<?php echo esc_js( $looprandom ); ?><?php the_ID(); ?> .openlightbox').click(function (e) {
                    e.preventDefault();
                    jQuery(this).lightGallery({
                        ...
                        dynamicEl: [
							...
							<?php if ( isset( $link['dvvideourl'] ) ) {
							$videourl = esc_js( $link['dvvideourl'] );
						} ?>
							<?php if ( isset( $link['dvvideotitle'] ) ) {
							$videotitle = esc_js( $link['dvvideotitle'] );
						} ?>
                            {
                                "src": "<?php if ( isset( $link['dvvideourl'] ) ) {
									echo esc_js( $videourl );
								} ?>",
                                "subHtml": "<?php if ( isset( $link['dvvideotitle'] ) ) {
									echo esc_js( $videotitle );
								} ?>",
                            },
							<?php } ?>
                        ]
